...
const db = knex({
  ...
  connection: {
    ...
    database: process.env.MYSQL_DB || 'newvaccine',
...
app.get('/list', async (req, res) => {
  ...
  const row = await db('register').where({ r_id: req.query.rid })
  res.send({
    datas: row[0],
    status: 1
  })
...
app.get('/listg', async (req, res) => {
  try {
    const rows = await req.db('group').select('*').orderBy('group.g_code', 'desc')
